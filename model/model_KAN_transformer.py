...
class PAMNet_KAN(nn.Module):
    def __init__(self, config: Config, num_spherical=7, num_radial=6, envelope_exponent=5):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.dim, nhead=8)
        ...
        self.mlp_sbf1 = KAN([num_spherical * num_radial, 32,self.dim])
        self.mlp_sbf2 = KAN([num_spherical * num_radial,  32,self.dim])#8
    ...
    def forward(self, data):
        ...
        # 在进行消息传递之前，通过 Transformer 编码器处理特征
        x = x.unsqueeze(1)  # 添加时间维度以适应 Transformer 的输入格式 [seq_len, batch_size, d_model]
        x = self.transformer_encoder(x)  # 应用 Transformer 编码器
        x = x.squeeze(1)  # 移除时间维度

        idx_i, idx_j, idx_k, idx_kj, idx_ji, idx_i_pair, idx_j1_pair, idx_j2_pair, idx_jj_pair, idx_ji_pair = self.indices(
        ...
        #att_score = torch.cat((torch.cat(att_score_local, 0), torch.cat(att_score_local, 0)), -1)
        ...
        #out = torch.cat((torch.cat(out_local, 0), torch.cat(out_local, 0)), -1)
...
class PAMNet_s(nn.Module):
    def __init__(self, config: Config, num_spherical=7, num_radial=6, envelope_exponent=5):
        ...
        self.mlp_rbf_g = KAN([16,  8,self.dim])
        self.mlp_rbf_l = KAN([16,  8,self.dim])
        self.mlp_sbf = KAN([num_spherical * num_radial, 8, self.dim])
