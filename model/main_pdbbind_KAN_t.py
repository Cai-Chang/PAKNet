...
from model_KAN_transformer import PAMNet_KAN, Config
...
def test(model, loader, device):
    ...
    return rmse(y, pred), mae(y, pred), sd(y, pred), pearson(y, pred), spearman(y,pred)
...
def main():
    ...
    # Split dataset
    idx_train, idx_val = train_test_split(np.arange(len(refined_dataset)), test_size=0.1, shuffle=False,
    ...
    model = PAMNet_KAN(config).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        train_rmse, train_mae, train_sd, train_p ,train_R= test(model, train_loader, device)
        val_rmse, val_mae, val_sd, val_p ,val_R= test(model, val_loader, device)

        if best_val_rmse is None or val_rmse < best_val_rmse:
            test_rmse, test_mae, test_sd, test_p, test_R= test(model, test_loader, device)
            best_val_rmse = val_rmse

        print('Epoch: {:03d}, Train RMSE: {:.7f}, Train MAE: {:.7f}, Train SD: {:.7f}, Train P: {:.7f}, Train R:{:.7f},\
            Test RMSE: {:.7f}, Test MAE: {:.7f}, Test SD: {:.7f}, Test P: {:.7f}, Test R:{:.7f}'.format(epoch + 1, train_rmse,
                                                                                         train_mae, train_sd, train_p,train_R,
                                                                                         test_rmse, test_mae, test_sd,
                                                                                         test_p,test_R))
